...
onBeforeRouteUpdate(()=>{
    setTimeout(() => {
        if (!store.loadTasks&&!route.params.idTask) {
            console.log(route.params.idFollowup);
            store.listAllTasks(route.params.idFollowup);
        }
    }, 100);
});
...
<template>
    <main id="main-view">
            ...
            <v-card elevation="0" class="header" style="border-radius: 10px;">
                <v-list-item style="width: 100%; margin: 0; padding: 0 .7rem;">
                    ...
                    <template v-slot:append>
                        <v-btn :loading="store.loadTasks" class="mr-3" size="38" icon="mdi-sync" variant="elevated" style="border-radius: 10px; color: var(--text-color-dark);" @click="store.listAllTasks(route.params.idFollowup)" >
                        ...
                        <v-btn class="mr-3" size="38" icon="mdi-filter-variant" variant="elevated" style="border-radius: 10px; color: var(--text-color-dark);"></v-btn>
                        <!-- <Filter /> -->
                        ...
                        <v-btn class="mr-3" size="38" icon="mdi-account-multiple" variant="elevated" style="border-radius: 10px; color: var(--text-color-dark);"></v-btn>

                         <FollowupConfig title="Configurações do seguimento" :new="false" :followupId="route.params.idFollowup" :obj="JSON.stringify(store.followup.filter(f=>f._id===route.params.idFollowup)[0])" />
                    </template>
                </v-list-item>
            </v-card>
        ...
        <div id="main-phases">
            <Phase v-for="phase, index in store.followup.filter(p=>p._id===route.params.idFollowup)[0].phases" :key="index" :title="phase.title" :id="phase.id" :tasks="store.tasks[`${route.params.idFollowup}`].filter(t=>t.phase_id===phase.id&&t.followup_id===route.params.idFollowup)" :search="search_task" />
        </div>
    </main>
</template>
