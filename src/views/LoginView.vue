...
<template>
  <div v-show="!store.auth" class="body-login" style="display: flex; width: 100%; height: 100%; align-items: center; justify-content: center; color: var(--text-color-dark)">
      ...
      <v-card class="mx-auto pa-12 pb-8" max-width="800" elevation="8" rounded="lg" style="background-color: var(--bg-color-light); color: var(--text-color-dark); position: relative; overflow-y: auto; border-radius: 0 !important;">
        <v-row dense>
          <v-col cols="12" sm="6">
            <v-empty-state>
          ...
          <v-col cols="12" sm="6">
            <div class="text-subtitle-1 mb-8" style="display: flex; align-items: center; justify-content: center; gap: 1rem">
              ...
              <h2 style="font-size: 27px; font-weight: 500">Project Manager</h2>
            </div>
  ...
  <!-- Mensagem de Erro -->
  <v-snackbar class="mb-10" v-model="snackbar" :timeout="timeout" style="color: darkred" variant="tonal">
    <h4>{{ message_error }}</h4>
