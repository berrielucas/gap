...
<template>
  <div v-show="!store.auth"
    style="display: flex; width: 100%; height: 100%; align-items: center; justify-content: center; color: var(--text-color-dark);">

    <div
      style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 50%; padding: 4rem;">
      <v-empty-state>
    ...
    <div style="width: 50%; display: flex; align-items: center; justify-content: center;">
      <v-card class="mx-auto pa-12 pb-8" elevation="8" max-width="460" rounded="lg"
        ...
        <v-window v-model="option">
          <v-window-item value="Login">
            <v-container fluid style="padding: 0;">
              ...
              <v-text-field density="compact" placeholder="Email" prepend-inner-icon="mdi-email-outline"
                variant="outlined" v-model="dataLogin.email"></v-text-field>

              <v-text-field :append-inner-icon="!visible ? 'mdi-eye-off' : 'mdi-eye'"
                :type="visible ? 'text' : 'password'" density="compact" placeholder="Senha"
                prepend-inner-icon="mdi-lock-outline" variant="outlined" @click:append-inner="visible = !visible"
                v-model="dataLogin.password"></v-text-field>
              ...
              <v-btn class="mb-5" size="large" block
                style="background-color: var(--button-color); color: var(--text-color-light);" @click="login()">
                <button type="submit">Entrar</button>
              </v-btn>

              <v-card-text class="text-center">
                <a class="text-decoration-none" rel="noopener noreferrer" @click="changeOption('Register')"
          ...
          <v-window-item value="Register">
            <v-container fluid style="padding: 0;">
              ...
              <v-text-field density="compact" placeholder="Nome" prepend-inner-icon="mdi-account-outline"
                variant="outlined" v-model="dataRegister.name"></v-text-field>

              <v-text-field density="compact" placeholder="Email" prepend-inner-icon="mdi-email-outline"
                variant="outlined" v-model="dataRegister.email"></v-text-field>

              <v-text-field :append-inner-icon="!visible ? 'mdi-eye-off' : 'mdi-eye'"
                :type="visible ? 'text' : 'password'" density="compact" placeholder="Senha"
                prepend-inner-icon="mdi-lock-outline" variant="outlined" @click:append-inner="visible = !visible"
                v-model="dataRegister.password"></v-text-field>
              ...
              <v-btn class="mb-5" size="large" block
                style="background-color: var(--button-color); color: var(--text-color-light);" @click="load = true">
                <button>Registrar-se</button>
              </v-btn>

              <v-card-text class="text-center">
                <a class="text-blue text-decoration-none" rel="noopener noreferrer" @click="changeOption('Login')"
                  style="color: var(--button-color); cursor: pointer;">
