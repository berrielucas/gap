...
<template>
  ...
  <v-dialog v-model="dialog" style="padding: 0; margin: 0; width: auto" persistent>
    ...
    >
      ...
      <v-card-title elevation="3" style="display: flex; align-items: start; justify-content: end; padding: 0.2rem 0.5rem 0 0.5rem">
        <span>
          <v-icon class="mt-5 ml-4" size="29">mdi-note-text-outline</v-icon>
        </span>
        <v-textarea v-model="task.title" class="title-task" style="width: 100%" placeholder="Nova Tarefa" single-line hide-details maxlength="60" variant="solo" rows="1" auto-grow shaped></v-textarea>
        <span>
          <v-btn icon="mdi-close" class="mt-4 mr-4" variant="text" size="30" density="comfortable" style="border-radius: 10px" @click="closeTask()"></v-btn>
        </span>
      </v-card-title>
      ...
      <v-card-text style="display: flex; gap: 1rem; padding: 0 0 1rem 1rem; overflow: hidden; position: relative">
        <v-row>
          ...
          <v-col cols="12" sm="8" style="padding-right: 0; display: flex; flex-direction: column; height: 100%; position: relative">
            <div style="height: 100%; overflow-y: auto; padding-bottom: 100px">
              ...
              <div class="mt-6 mb-6" style="display: flex">
                <span style="color: #a7a7a7">
                  <v-icon class="mt-2 ml-2" size="29">mdi-checkbox-multiple-marked-outline</v-icon>
                </span>
                <div style="width: 100%">
                  <h3 class="mt-2 ml-4 mb-3" style="color: #aaaaaa; font-weight: 400; display: flex; align-items: center">
                    <b>Sub Tarefas</b>
              ...
              <v-scale-transition class="py-0" tag="v-list">
                <v-list-item v-if="task.subTasks.length === 0">
                  <p class="mt-6 mb-6" style="opacity: 0.5; text-align: center">Nenhuma tarefa vinculada</p>
                </v-list-item>
              </v-scale-transition>
            ...
            <!-- Input de Comentario -->
            <div class="ml-4 mr-4 mb-3 mt-3" style="display: flex; align-items: end; gap: 0.5rem; position: absolute; bottom: 0 !important; left: 0; right: 0">
              <div style="display: flex; height: auto; align-items: center; flex-grow: 1; color: var(--text-color-dark); border: solid 1px #bfbfbf; border-radius: 10px; overflow: hidden">
                <v-textarea density="compact" placeholder="Comente aqui..." variant="solo" single-line hide-details max-rows="4" rows="1" auto-grow shaped> </v-textarea>
              </div>
              <v-btn
                size="42"
                icon="mdi-send"
                variant="text"
                style="display: flex; border-radius: 10px; color: var(--text-color-light); background-color: var(--button-color); font-size: 15px"
              ></v-btn>
            </div>
          </v-col>
          ...
          <v-col cols="12" sm="4" style="display: flex; flex-direction: column; border-left: solid 1px #dedede; max-height: 100%; width: 100%; overflow-y: auto">
            ...
            <p v-if="task.properties.length === 0" class="mt-6" style="opacity: 0.5; text-align: center">Sem propriedades</p>
